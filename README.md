...
初次启动会加载约 182 根小时 K 线。首次成功建立 LP 之前，不要求等待 6 小时冷却或累计 6 个新的健康小时；账户对账、行情风险和保证金检查仍须通过。行情若符合下跌条件，保持 `Paused`、稳定币等待，是预期行为。首次成功建立任意一层 LP 后，暂停恢复继续要求原来的冷却及健康小时条件，重启不会重置这一记录。旧版状态的迁移参数见 [首次建仓与暂停恢复](docs/recovery.md#首次建仓与暂停恢复)。Ctrl-C 停止进程，**不会自动平掉现有实盘仓位**。
...
- 首次建仓按完整 LP 预算执行，不等 6 小时冷却或 6 个新的健康小时；历史行情必须完整且当前市场检查通过。曾经建立 LP 后的暂停恢复至少冷却 6 小时，连续 6 个新完成小时满足：波动比 < 1.2、无下跌信号、没有新低、价格不低于快均线。恢复 25% LP；持续健康每 4 小时增加 25%。已有区间通过 increase 增加资金。
