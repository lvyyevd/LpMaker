...
| 读取超时、断开、5xx | 只读接口有限重试；仍失败则标记 degraded，默认 5 秒后重新加载检查点、核对 pending、实际持仓和订单，再恢复决策。遗留策略挂单按恢复流程撤销。 |
| EVM RPC 429 / 明确的 JSON-RPC 频率限制 | 同端点共享节流与退避，默认 30、60、120、240、300 秒逐步等待，尊重更长的 Retry-After；重建客户端不清零退避，监控和 nonce 定时查询暂停访问该节点。详见 [RPC 限流恢复](rpc-rate-limits.md)。 |
